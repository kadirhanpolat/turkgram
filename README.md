...
- **analyze() perf ~48× ✅ (2026-07-19)** — tam-leksikon 165→3.5ms; dört recall-güvenli opt (adj O(roots)→O(aday); `_make_frozen` lambda-elenme; copula+conjugate person-ending prune). Davranış-korunumlu (copula 40k + conjugate 50k exhaustive diferansiyel 0 recall-miss).
...
**4279 test** (slow hariç). Round-trip tam süpürme `-m slow` ile: `pytest -m slow`.
