...
- **m-İkileme nominal-yeniden-kurulum (NP) ✅ (2026-07-18)** — AdvP'nin nominal kardeşi; parser NOUN-tabanlı m-ikilemeyi tek `NP` öbeği kurar:
  ...
  - R9 R8'den sonra, R0/R1'den önce. Dependency: taban isim → fiile `nsubj`, reduplikant → başa `compound:redup` (upos=NOUN, UD nominal-compound). ADJ-taban (`güzel müzel`) kapsam dışı (V1, defer).
  - Tasarım: `docs/superpowers/specs/2026-07-18-m-ikileme-nominal-design.md`; plan: `docs/superpowers/plans/2026-07-18-m-ikileme-nominal.md`. Hakem: 0 çökme + adversarial (HIGH sertleştirme giderildi). **+5 test (4107).**
...
**4107 test** (slow hariç). Round-trip tam süpürme `-m slow` ile: `pytest -m slow`.
