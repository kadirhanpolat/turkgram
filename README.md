...
- **📋 Planlanan — İkileme adverbial-yeniden-kurulum (AdvP)** — tasarım + implementasyon planı hazır, uygulama bekliyor:
  - Tam ikileme (`yavaş yavaş`) + ulaç ikilemesi (`koşa koşa`) → yeni `AdvP` öbeği; parser R8_redup kuralı (yüzey-tabanlı, bitişik özdeş çift; NOUN-takip guard'ıyla adnominal `uzun uzun yollar` korunur). m-ikileme kapsam dışı.
  - Dependency: AdvP başı → fiile `advmod`, ikinci token → başa `compound:redup`.
  - Tasarım: `docs/superpowers/specs/2026-07-18-ikileme-adverbial-design.md`; plan: `docs/superpowers/plans/2026-07-18-ikileme-adverbial.md` (ikisi de çift-tur hakem onaylı).
...
**4096 test** (slow hariç). Round-trip tam süpürme `-m slow` ile: `pytest -m slow`.
